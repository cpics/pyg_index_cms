  ...
  <div class="app-container">
    ...
    <el-dialog v-if="dialogFormVisible" title="详情" :close-on-press-escape="false" :close-on-click-modal="false" :visible="dialogFormVisible" @close="close">
      <el-form>
        ...
        <el-form-item label="入驻需求">
          {{ oneInfo.requirement }}
        </el-form-item>
        <el-form-item label="入驻时间">
          {{ oneInfo.settled_date }}
        </el-form-item>
        <el-form-item label="入驻人数">
          {{ oneInfo.person_count }}
        </el-form-item>
...
<script>
import { FetchSettleds } from '@/api/apply'
export default {
  data() {
    return {
  ...
  methods: {
    async fetchFinances() {
      const res = await FetchSettleds()
      this.list = res.apply_settleds
    },
    showDetail(index) {
